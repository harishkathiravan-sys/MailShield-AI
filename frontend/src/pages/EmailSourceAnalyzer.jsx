...
import { FileText, AlertTriangle, CheckCircle, XCircle, Mail, ExternalLink, Shield, Link as LinkIcon, Globe, Terminal, Zap, ShieldCheck } from 'lucide-react';
...
const EmailSourceAnalyzer = () => {
  ...
  const loadDemo = () => {
    setSource(`Delivered-To: user@example.com
Received: by 2002:a05:620a:258b:b0:78a:5f3a:9e1b with SMTP id f11csp2415170qka;
        Mon, 1 Jan 2026 10:00:00 -0800 (PST)
From: "Security Alert" <alert@secure-bank-login.net>
To: user@example.com
Subject: URGENT: Security verification required
Date: Mon, 1 Jan 2026 10:00:00 +0000
Message-ID: <827346283746.12345@secure-bank-login.net>
MIME-Version: 1.0
Content-Type: text/plain; charset="UTF-8"

Your account has been accessed from a new location. Please verify your identity at: http://secure-bank-login.net/verify/92834`);
      ...
      <div className="min-h-screen py-12 px-4">
        <div className="max-w-6xl mx-auto">
          ...
          <motion.div
            ...
            className="bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border-2 border-yellow-500/50 rounded-lg p-4 mb-6"
          >
          ...
          {results.masked_urls && results.masked_urls.length > 0 && (
            <motion.div
              ...
              className="cyber-card border-2 border-red-500/50 bg-red-500/10 mb-6"
            >
          ...
          >
            <button
              onClick={() => { setResults(null); setSource(''); }}
  ...
  return (
    ...
    >
      <div className="mb-12">
        <h1 className="text-4xl font-bold text-slate-900 mb-4">Email Source Analyzer</h1>
        <p className="text-lg text-slate-500">Paste the complete email source to extract hidden URLs and detect phishing</p>
      </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div className="space-y-6">
              <SourceGuide platform="Gmail" steps='Open email → ⋮ (three dots) → "Show original" → Copy to clipboard' />
              <SourceGuide platform="Outlook" steps='Open email → ⋯ (More actions) → "View message source"' />
              <SourceGuide platform="Yahoo" steps='Open email → More → "View raw message"' />
            </div>
          </div>
          ...
          <div className="bg-indigo-50 rounded-3xl p-8 border border-indigo-100">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h4 className="font-bold text-indigo-900">Header Forensics</h4>
            </div>
            <p className="text-sm text-indigo-700 leading-relaxed">Analyze hop-by-hop delivery headers to identify spoofed origins and verify SPF, DKIM, and DMARC authentication results for complete security verification.</p>
          </div>
        </div>
        ...
        <div className="lg:col-span-8">
          <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
            <div className="bg-slate-50 px-8 py-4 border-b border-slate-200 flex justify-between items-center">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">Email Source Input</span>
...
..."
                className="w-full p-6 rounded-2xl border border-slate-100 bg-slate-50 font-mono text-sm text-slate-600 focus:ring-0 focus:border-slate-200 outline-none transition-all resize-none"
              />

              <div className="flex flex-col sm:flex-row gap-4 mt-8">
                <button
                  onClick={handleAnalyze}
                  className="flex-[2] bg-slate-900 text-white py-5 rounded-2xl font-bold hover:bg-slate-800 transition-all shadow-xl shadow-slate-200 flex items-center justify-center gap-3"
                >
...
const SourceGuide = ({ platform, steps }) => (
  <div>
